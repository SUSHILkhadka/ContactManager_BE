...
export const getAllContactsByUserId = (req: IRequestWithTokenData, res: Response, next: NextFunction) => {
  const userId = req.id;
  if (!userId) {
    return next(new CustomError('id of user in token data is required', StatusCodes.BAD_REQUEST));
  }
...
export const deleteContact = (req: IRequestWithTokenData, res: Response, next: NextFunction) => {
  const userId = req.id;
  const id = req.params.contactId;
  if (!userId || !id) {
    return next(new CustomError('id of user in token data is required', StatusCodes.BAD_REQUEST));
